...
function n_printPageTitle($is_front,$title,$title_prefix,$title_suffix){
  if ($is_front){
    return '<h1>Welcome to the '.(defined('myOrg_name') ? myOrg_name : '').' <br/>Computer Aided Administration</h1>';
  }else{
    // shorten the title if it is too long
    if (isset($GLOBALS['custom_title'])) $title = $GLOBALS['custom_title'];
 ...
 */
function nordita_tpl_css($f,$type='css',$print=True){
  static $theme = 'nordita';
  ...
  if (!is_file($reply)){
    ...
    if (class_exists('myPear',0)) myPear::ERROR($msg);
    else  print "$msg<br>";
  }
 ...
 *
 */
function nordita_tpl_info(){
  $info = array();
  /*
  if (defined('cnf_dev')) foreach (array(// 'cnf_localhost'=>'localhost',
					 'cnf_demo'     =>'Demo version',
					 'cnf_dev'      =>'Developer mode',
					 ) as $opt=>$txt) if (constant($opt)) $info[] = $txt;
  */
  return (empty($info)
	  ? ""
...
function nordita_tpl_login(){
  ...
  if (class_exists('bAuth',0)){
    ...
    if (is_object(bAuth::$av) && function_exists('myPear_access')){
      ...
      $reply = x("li","<a href='".b_url::same("?resetcache_once=1&login=yes&$flavor")."'><img alt='login' src=".nordita_tpl_css('login','png',0)." /></a>");
    }
  }
  return $reply;
}
