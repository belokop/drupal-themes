 ...
 */
  ...
  */
function sh_preprocess_html(&$vars) {
  ...
  if (isset($vars['page']['sidebar_first']['system_navigation'])){
	...
	foreach($nav['#below'] as $k2=>$nav2){
	  if (is_numeric($k2) && is_array($nav2)){
	    $sh_nav_level2[] = array('#href' => $nav2['#href'],
					  '#title'=> $nav2['#title']);
	  }
	}
      }
 ...
 */
function sh_menu_tree($vars) {
  $tree  = str_replace("><ul",">\n<ul",str_replace("><li",">\n<li",$vars['tree']));
  return "<ul class='menu page-load-progress'>$tree</ul>";
}

/**
 * Change button to Post instead of Save
 */
function sh_form_comment_form_alter(&$form, &$form_state, &$form_id) {
